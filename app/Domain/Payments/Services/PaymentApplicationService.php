...
class PaymentApplicationService
{
     ...
     */
    public function applyPaymentAutomatically(Payment $payment): Collection
    {
        $this->validatePaymentCanBeApplied($payment);
        ...
        return DB::transaction(function () use ($payment, $availableCharges): Collection {
            $allocations = new Collection;
            $remainingPayment = round((float) $payment->unapplied_amount, 2);
            ...
            foreach ($availableCharges as $charge) {
                ...
                $allocationAmount = min($remainingPayment, $chargeRemaining);
            ...
            $this->updatePaymentFromAllocations($payment);
     ...
     * @throws DomainException
     */
    public function applyPaymentManually(Payment $payment, array $chargeAllocations): Collection
    {
        $this->validatePaymentCanBeApplied($payment);
        ...
        return DB::transaction(function () use ($payment, $chargeAllocations): Collection {
            $allocations = new Collection;
    ...
    public function reverseAllocation(PaymentAllocation $allocation): void
    {
        DB::transaction(function () use ($allocation): void {
            $payment = Payment::query()->lockForUpdate()->findOrFail($allocation->payment_id);
            ...
            $newStatus = $this->calculatePaymentStatusFromAmounts($payment->amount, $newAppliedAmount, $newUnappliedAmount);
     ...
     */
    public function getAvailableCharges(Payment $payment): Collection
    {
     ...
     * }
     */
    public function previewAutomaticAllocation(Payment $payment): array
    {
        $availableCharges = $this->getAvailableCharges($payment);
        $remainingPayment = round((float) $payment->unapplied_amount, 2);
        $allocations = [];
     ...
     * @throws DomainException
     */
    private function validatePaymentCanBeApplied(Payment $payment): void
    {
        if ($payment->status === PaymentStatus::Cancelled) {
            throw new DomainException('No se puede aplicar un pago cancelado.');
        }

        if ($payment->status === PaymentStatus::FullyApplied) {
     ...
     * @throws DomainException
     */
    private function validateManualAllocations(Payment $payment, array $chargeAllocations): void
    {
        $totalRequested = 0.0;
     ...
     */
    private function createAllocation(Payment $payment, PartnerCharge $charge, float $amount): PaymentAllocation
    {
     ...
     */
    private function updatePaymentFromAllocations(Payment $payment): void
    {
        $totalApplied = PaymentAllocation::query()
     ...
     */
    private function calculatePaymentStatus(Payment $payment): PaymentStatus
    {
        $unapplied = round((float) $payment->unapplied_amount, 2);
        $applied = round((float) $payment->applied_amount, 2);

        return $this->calculatePaymentStatusFromAmounts($payment->amount, $applied, $unapplied);
    }

    /**
     * Calculate payment status from amounts (for use during reversal).
     */
    private function calculatePaymentStatusFromAmounts(float $total, float $applied, float $unapplied): PaymentStatus
    {
        $unapplied = round($unapplied, 2);
        $applied = round($applied, 2);

        if ($unapplied <= 0.0 && $applied > 0.0) {
            return PaymentStatus::FullyApplied;
        }
        ...
        return PaymentStatus::PendingApplication;
    }
}
